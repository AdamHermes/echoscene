...
def main():
    ...
    artifact_dir = "/Users/lehoangan/.gemini/antigravity-cli/brain/5f92a6bd-eed1-4da1-a3a1-307c85b23437"
    ...
    if not scene_rel:
        print(f"Scene {target_scene} not found in relationships!")
        return
    ...
    if target_scene not in box_data:
    ...
    # Save Layout Plots
    layout_local_path = os.path.join(base_dir, "debug_bbox", f"{target_scene}_input_layout.png")
    ...
    # Add nodes (excluding floor to keep graph cleaner, or include it)
    for k, name in objects_map.items():
        node_id = f"{name}_{k}"
    ...
    # Choose a layout
    pos = nx.shell_layout(G)  # Shell layout keeps it neat
    ...
    # Save Graph Plots
    graph_local_path = os.path.join(base_dir, "debug_bbox", f"{target_scene}_input_graph.png")
