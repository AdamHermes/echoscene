...
new_logic = """
    # Convert objects to walls so AI2-THOR NavMesh baker respects them
    procthor_objects = []
    shifted_obj_polys = []
    ...
    for idx, obj in enumerate(scene_data['objects']):
        ...
        # Calculate shifted poly for spawn check and walls
        ...
        # Add walls for the object
        for i in range(len(world_corners)):
            va = world_corners[i]
            vb = world_corners[(i+1)%len(world_corners)]
            wall_id = f"wall|1|{va[0]:.3f}|{va[1]:.3f}|{vb[0]:.3f}|{vb[1]:.3f}_{idx}_{i}"
            walls.append({
                "id": wall_id,
                "roomId": "room|1", # Put in a dummy room to not mess up room|0 bounds
    ...
    house["rooms"] = [{
        ...
        "children": [],
        "floorMaterial": {"name": "WoodFloorsCross"},
        "floorPolygon": polygon, # Just copy the same floor so room|1 is valid
        "ceilings": []
    }]
    house["walls"] = walls
...
pattern = r"    # Convert objects to physical ProcTHOR objects.*?return house"
