...
for folder in folders:
    ...
    subprocess.run(["python", "convert_echoscene_to_procthor.py", "--bbox_path", final_json, "--out_dir", out_dir], check=True)
    
    # 3. Evaluate walkability with AI2-THOR Unity engine
    print("Running Unity walkability evaluation...")
    subprocess.run(["python", "eval_walkability.py", "--scenes_dir", out_dir], check=True)
