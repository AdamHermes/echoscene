...
```
This script takes the updated collision bounding boxes from `physcene_collision_resolved.json`, re-aligns the base meshes generated by the diffusion models, regenerates the `.glb` files with the new layout, and creates updated top-down `render_imgs` in the `complete_released_full_model_post_processed` folder.

### Physical Collision Evaluation (ColObj & ColScene)
To calculate **ColObj** (Object Collision Rate) and **ColScene** (Scene Collision Rate) metrics across generated room layouts (both raw input predictions and post-processed collision resolved predictions):

#### 1. Command Line Interface (CLI)
Run the dedicated collision evaluation script from the `echoscene` directory:
```bash
# Evaluate raw input predictions (first 190 rooms)
python scripts/eval_collision.py --json current_works/to_be_merged/complete_released_full_model/vis/2050/physcene_collision_input.json --max_rooms 190

# Evaluate post-processed collision-resolved predictions (first 190 rooms)
python scripts/eval_collision.py --json current_works/to_be_merged/complete_released_full_model/vis/2050/physcene_collision_resolved.json --max_rooms 190
```
*Options:*
- `--json`: Path to target prediction JSON file.
- `--max_rooms`: Number of rooms to evaluate (default: `190`, set to `0` to evaluate all available rooms).

#### 2. Jupyter Notebook Evaluation
Open [`PhyScene.ipynb`](../PhyScene.ipynb) and run Cell 8:
```python
from evaluate_furniture_collisions import evaluate_furniture_collisions

# Run evaluation on any prediction JSON file
results = evaluate_furniture_collisions(
    "current_works/to_be_merged/complete_released_full_model/vis/2050/physcene_collision_resolved.json",
    max_rooms=190
)
```
